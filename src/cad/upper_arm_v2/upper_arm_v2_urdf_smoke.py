...
IMPROVED_MESH_NAME = "upper_arm_v2_ai_rebuild_m.stl"
...
def prepare_urdf(project_root: Path, upper_arm_dir: Path) -> tuple[Path, Path, dict[str, object]]:
    ...
    mesh_stats = scale_stl_to_meters(upper_arm_dir / "upper_arm_v2_ai_rebuild.stl", improved_mesh)
    ...
    output_urdf = out_dir / "so101_upper_arm_v2_visual_smoke.urdf"
...
def write_report(
    ...
    lines = [
        ...
        f"- V2 visual smoke URDF：`{improved_urdf}`",
        "",
        "## Mesh 缩放检查",
        "",
        f"- V2 原始 STL 包围盒：{mesh_stats['source_extents_raw']}",
        ...
        f"- V2 关节数：{smoke['improved_joint_count']}",
...
def main() -> None:
    ...
    write_motion_csv(out_dir / "upper_arm_v2_urdf_smoke_motion.csv", smoke["rows"])
    plot_smoke(out_dir / "upper_arm_v2_urdf_smoke_plot.png", mesh_stats, smoke)
    write_report(
        out_dir / "upper_arm_v2_urdf_smoke_report_zh.md",
        official_urdf,
        improved_urdf,
        mesh_stats,
        smoke,
    ...
    print(f"Saved smoke report: {out_dir / 'upper_arm_v2_urdf_smoke_report_zh.md'}")
    print(f"Saved smoke plot: {out_dir / 'upper_arm_v2_urdf_smoke_plot.png'}")
