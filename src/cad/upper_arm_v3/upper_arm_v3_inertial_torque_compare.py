...
def read_volume_info(validation_json: Path) -> dict[str, float]:
    ...
    v3 = data["upper_arm_v3"]
    official_volume = float(official["volume"])
    v1_volume = float(v1["volume"])
    v2_volume = float(v2["volume"])
    v3_volume = float(v3["volume"])
    if min(official_volume, v1_volume, v2_volume, v3_volume) <= 0:
    ...
    return {
        ...
        "v3_estimated_mass_g_pla": float(v3["estimated_mass_g_pla"]),
    }
...
def prepare_scaled_inertial_urdf(
    ...
    visual_smoke_urdf = visual_smoke_dir / "so101_upper_arm_v3_visual_smoke.urdf"
    ...
    output_urdf = out_dir / "so101_upper_arm_v3_inertial_scaled.urdf"
...
def run_existing_same_seed_rows(project_root: Path, variant: str, samples: int, seed: int) -> list[dict[str, object]]:
    official_urdf = project_root / "00_source_snapshot" / "Simulation_SO101" / "so101_new_calib.urdf"
    if variant == "v1":
        improved_urdf = (
            project_root
        ...
        improved_urdf = (
            ...
            / "upper_arm_v2"
            / "inertial_compare"
            / "so101_upper_arm_v2_inertial_scaled.urdf"
        )
    else:
        raise ValueError(f"unsupported variant: {variant}")
    if not improved_urdf.exists():
        return []
    comparison = run_comparison(official_urdf, improved_urdf, samples, seed)
...
def plot_comparison(path: Path, rows: list[dict[str, object]]) -> None:
    ...
    ax_bar.bar(x + width / 2, imp_p95, width, label="upper_arm_v3")
...
def v2_value(v2_rows: list[dict[str, object]], joint: str, key: str) -> float | None:
    for row in v2_rows:
        if abs(float(row["payload_kg"]) - REPORT_PAYLOAD_KG) < 1e-9 and row["joint"] == joint:
            return float(row[key])
    return None
...
def write_report(
    ...
    lines = [
        ...
        f"- V3 upper arm STL 体积：{volume_info['v3_volume_mm3']:.2f} mm^3",
        f"- V1 / 官方体积比例：{volume_info['v1_volume_ratio']:.4f}",
        f"- V2 / 官方体积比例：{volume_info['v2_volume_ratio']:.4f}",
        f"- V3 / 官方体积比例：{volume_info['v3_volume_ratio']:.4f}",
        f"- V2 / V1 体积比例：{volume_info['v2_vs_v1_volume_ratio']:.4f}",
        f"- V3 / V2 体积比例：{volume_info['v3_vs_v2_volume_ratio']:.4f}",
        f"- 官方 URDF `upper_arm_link` 质量：{float(inertial_info['official_upper_arm_link_mass_kg']):.6f} kg",
        f"- V3 screening 质量：{float(inertial_info['scaled_upper_arm_link_mass_kg']):.6f} kg",
        f"- V3 相对官方质量变化：{float(inertial_info['mass_delta_kg']):.6f} kg ({float(inertial_info['mass_delta_pct']):.2f}%)",
        "",
    ...
    if v1_rows and v2_rows:
        ...
        for row in report_rows:
            ...
            v2_p95 = v2_value(v2_rows, joint, "improved_p95_abs_nm")
            if v1_p95 is None or v2_p95 is None:
                continue
            v3_p95 = float(row["improved_p95_abs_nm"])
            lines.append(
                f"| {joint} | {float(row['official_p95_abs_nm']):.4f} | "
                f"{v1_p95:.4f} | {v2_p95:.4f} | {v3_p95:.4f} | {v3_p95 - v2_p95:.4f} |"
            )
...
def main() -> None:
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    volume_info = read_volume_info(args.upper_arm_dir / "upper_arm_v3_validation.json")
    official_urdf, improved_urdf, inertial_info = prepare_scaled_inertial_urdf(
        args.project_root, args.upper_arm_dir, volume_info["v3_volume_ratio"]
    )
    comparison = run_comparison(official_urdf, improved_urdf, args.samples, args.seed)
    rows = build_stats_rows(comparison)
    v1_rows = run_existing_same_seed_rows(args.project_root, "v1", args.samples, args.seed)
    v2_rows = run_existing_same_seed_rows(args.project_root, "v2", args.samples, args.seed)
    if not rows:
        raise RuntimeError("No torque rows computed for V3")
    write_stats_csv(out_dir / "upper_arm_v3_torque_comparison.csv", rows)
    if v1_rows:
        write_stats_csv(out_dir / "upper_arm_v1_same_seed_torque_comparison.csv", v1_rows)
    if v2_rows:
        write_stats_csv(out_dir / "upper_arm_v2_same_seed_torque_comparison.csv", v2_rows)
    plot_comparison(out_dir / "upper_arm_v3_torque_comparison_plot.png", rows)
    write_report(
        out_dir / "upper_arm_v3_torque_comparison_report_zh.md",
        volume_info,
        inertial_info,
        rows,
        v1_rows,
    ...
    (out_dir / "upper_arm_v3_inertial_update.json").write_text(
    ...
    print(f"Saved torque comparison report: {out_dir / 'upper_arm_v3_torque_comparison_report_zh.md'}")
    print(f"Saved torque comparison plot: {out_dir / 'upper_arm_v3_torque_comparison_plot.png'}")
